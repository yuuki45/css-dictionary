...
詳細な技術ドキュメントは[docsフォルダ](./docs/)にあります：

- **[アーキテクチャドキュメント](./docs/architecture.md)** - プロジェクト全体のアーキテクチャと技術スタック
- **[データ構造ドキュメント](./docs/data-structure.md)** - データ型とスキーマの詳細
- **[コンポーネント設計ドキュメント](./docs/components.md)** - 全コンポーネントの設計と使用方法
- **[ルーティング設計ドキュメント](./docs/routing.md)** - URL構造とルーティング実装
- **[開発ガイドライン](./docs/development-guide.md)** - コーディング規約とベストプラクティス

### 💡 コンテキストエンジニアリングに基づく開発

このプロジェクトでは、**コンテキストエンジニアリング**の考え方を採用しています。実装を進める際は、以下のアプローチを推奨します：

1. **ドキュメントを参照コンテキストとして活用**
   - 実装前に関連するドキュメントを確認
   - 既存のパターンや設計思想を理解
   - AIアシスタント（Claude等）に開発を依頼する際は、関連ドキュメントを明示的に参照

2. **一貫性のある実装**
   - コーディング規約とベストプラクティスを遵守
   - 既存のコンポーネント設計パターンに従う
   - データ構造やルーティング設計との整合性を保つ

3. **ドキュメント駆動開発**
   - 新機能追加時は設計をドキュメントに反映
   - アーキテクチャ変更時はドキュメントを先に更新
   - ドキュメントとコードの同期を維持
