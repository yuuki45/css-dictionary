...
CSSProperty {
  ...
  examples: CodeExample[], relatedProperties,
  browserSupport: BrowserSupport,  // 構造化（W3C Baseline準拠）
  aiNotes?, promptExamples?, searchAliases?, mdnUrl?  // AI協働向け（Phase 3以降で投入）
}

BrowserSupport {
  chrome?, firefox?, safari?, edge?,  // 初対応バージョン文字列
  baseline: 'widely' | 'newly' | 'limited',
  baselineLowDate?, note?
}
...
**データアクセス:** JSONを直接importせず `import { cssProperties } from '@/data/properties'` を使う（型付き中央モジュール）。
...
```json
{
  ...
  "name": "property-name",
  "category": "正準9カテゴリのいずれか（単一ソース: src/utils/categorySlug.ts）",
  "description": "SEO用に50文字以上",
  "syntax": "property: value;",
  "examples": [{ "code": "...", "description": "..." }],
  "relatedProperties": ["実在するIDのみ"],
  "browserSupport": { "chrome": "105", "firefox": "110", "safari": "16", "edge": "105", "baseline": "widely", "baselineLowDate": "2023-02" },
  "aiNotes": "AIがよく間違えるポイント（新規エントリでは必須）",
  "promptExamples": ["AIへの依頼文例（新規エントリでは必須）"]
}
```
- **browserSupportは手書きしない**。`scripts/lib/baseline-source.ts` の `lookupBaseline(id)` でweb-features（Baseline公式データ）から取得する
- 追加後は必ず `npm run validate` を実行（参照整合性・カテゴリ・構造を検証。CIでも実行される）
